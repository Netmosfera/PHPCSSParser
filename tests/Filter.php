...
final class Filter
{
    ...
    public function isFiltered(string $filename): bool
    {
        ...
        if (!isset($this->realPaths[$filename])) {
            $actualFilename = \realpath($filename);
            $this->realPaths[$filename] = $actualFilename;
        } else {
            $actualFilename = $this->realPaths[$filename];
        }

        return !isset($this->whitelistedFiles[$actualFilename]);
    }
